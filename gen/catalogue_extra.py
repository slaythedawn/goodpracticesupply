...
EXTRA_CATEGORIES = [
{
 ...
 'imgkey':'hand_wash',
 'alt':'A pump bottle of hand wash beside its refill',
 'intro':'The consumables a room gets through without anyone ordering them, until the day nobody can. Hand wash and hand towels at the basin, tissues on the desk, cotton wool for pressure after a needle. None of it is interesting and all of it stops the room working when it runs out, which is exactly what a repeating order is for. Interleaved hand towels rather than a roll, because they come out one at a time and the next person is not touching the stack. A roll in a busy room is a shared surface with a queue in front of it. Liquid hand wash in a pump with refills rather than replacement bottles, which is cheaper per wash and is less to throw away. Cotton wool in rolls rather than balls, so you tear what you need instead of using a whole one for a spot of pressure. Facial tissues because every consult room needs them and nobody puts them on a list until the box is empty in front of a patient. This is the category most worth putting on a schedule, because it is the one nobody notices until it is the only thing anybody notices.',
...
},
{
 ...
 'imgkey':'rigid_tape',
 'alt':'Three rolls of rigid zinc oxide strapping tape',
 'intro':'Rigid where you want a joint to stop, elastic where you want it to move and still be held. The two get bought together and used together. Rigid zinc oxide strapping tape for the anchor and the lock, elastic adhesive bandage over the top so the job survives a session rather than peeling at the first sweat. Neither is a substitute for the other and most taping jobs use both. Kinesiology tape, which people also search for as KT tape, is a different job again. It stretches with the skin rather than restricting a joint, it is worn for days rather than a session, and it is here because clinics order it in the same basket as the rest. Compression stockings are sized by measurement rather than by shoe size or dress size, and the class matters as much as the fit. Getting either wrong is the reason most pairs end up in a drawer. Sizing is on each product page. What class suits you is a question for your prescriber rather than something to pick off a chart. Rigid tape is also the most commonly over ordered line in this category, because a part-used roll looks nearly finished long before it is. Worth a look in the drawer before anyone reorders.',
