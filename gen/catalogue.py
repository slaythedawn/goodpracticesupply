 ...
 ]},
{
 ...
 'intro':'What a treatment room reaches for, in quantities that make sense for one person or for a clinic. The upgrade over a chemist plaster is the contact layer. A non adherent dressing has a contact layer the tissue does not stick to, so it lifts off rather than pulling at what is underneath. An ordinary adhesive pad does not, which is why a dressing change with the wrong pad is the part people dread. Saline is the routine choice for cleaning and antiseptics are kept for when they are indicated, which is a question for whoever is managing the wound rather than a rule to take off a shelf. We stock both. Sterile gauze swabs for cleaning and packing, island dressings where you want the pad and the adhesive in one piece, and conforming bandages to hold a dressing on a limb that moves. Fixation tape that holds without removing skin when it comes away, which matters more the older or thinner the skin is. A workplace first aid kit for the rooms that have to have one. [Storing supplies correctly](/learn/storing-supplies) matters more here than most people expect: a sterile dressing is only sterile while its pouch is intact and in date, and a box kept in a hot car is not the box you think it is.',
